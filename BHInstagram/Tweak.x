...
- (NSArray *)objectsForListAdapter:(id)arg1 {
    NSArray *origResult = %orig;
    NSMutableArray *orig = [origResult mutableCopy];
    ...
    return [BHIManager hideAds] ? removeAdsItemsInList([orig copy]) : [orig copy];
}
%end
...
// Hide explore grid (simplified)
%hook IGExploreGridViewController
- (void)viewDidLoad {
    // Not using view method to avoid forward declaration issues
    return; // Just return without calling %orig to prevent loading
}
%end
...
- (void)viewDidLoad {
    %orig;
    // Simplified to avoid MSHookIvar
}
%end

// Hide reels tab (simplified)
%hook IGTabBar
- (void)didMoveToWindow {
    %orig;
    // Simplified to avoid forward declaration issues
}
%end
